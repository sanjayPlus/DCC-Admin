...
function AllSwings() {
  ...
  const [swings, setSwing] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          axios
            .get(`${SERVER_URL}/admin/swing`, {
              headers: { "x-access-token": token },
            })
            .then((userResponse) => {
              if (userResponse.status === 200) {
            
                setSwing(userResponse?.data.swings);
              }
            })
            .catch((err) => {
  ...
  function handleDelete(id) {
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setSwing(swings?.filter((product) => product._id !== id));
        }
      })
      .catch((err) => {
        console.log(err.response.data);
                ...
                <div className="card">
                  <div className="card-body">
                    ...
                    <div className="table-responsive">
                      <table id="swings" className="table">
                        <thead>
                          <tr>
                            <th>Swings</th>
                            {/* <th>Edit</th> */}
