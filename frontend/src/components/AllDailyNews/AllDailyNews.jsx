...
function AllDailyNews() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          axios
            .get(`${SERVER_URL}/admin/daily-news`, {
              headers: { "x-access-token": token },
            })
            .then((userResponse) => {
              if (userResponse.status === 200) {
            
                setDailyNews(userResponse?.data.dailyNews);
              }
            })
            .catch((err) => {
  ...
  function handleDelete(id) {
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          setDailyNews(dailyNews?.filter((product) => product._id !== id));
        }
      })
      .catch((err) => {
        console.log(err.response.data);
    ...
    <>
      <div className="main-wrapper">
        ...
        <div className="page-wrapper">
          <NavBar name={name} setName={setName} link="all-daily-news" />
